...
public abstract class BaseTestCase extends TestCase
{
    ...
    static {
        ...
        if ("local".equals(testEnv)) {
            ...
            loginUrl = "http://192.168.101.131:8008/cross" + getLoginUrl(web);
            hostUrl = System.getProperty("hostUrl", "http://192.168.130.200:8080/hyserver");
        }
    ...
    private static User user = getDummyUser();;
    ...
    public static JsonNode HttpPost(String urlString, final String jsonContent) throws Exception
    {
    	return HttpRequestResult(HttpMethod.POST, urlString, jsonContent);
    }

    public static JsonNode HttpRequestResult(String httpMethod,String urlString, final String jsonContent) throws Exception
    {
        JsonNode node = HttpRequestFull(httpMethod, urlString, jsonContent);
        return node != null ? node.get(RetKey.Notification) : null;
    }

    public static JsonNode HttpRequestFull(String httpMethod,String urlString, final String jsonContent) throws Exception
    {
        String response = HttpRequestFullJson(httpMethod, urlString, jsonContent);
        return response != null ? Util.readTree(response) : null;
    }
