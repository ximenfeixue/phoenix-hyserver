...
//
//		List<Social> result = new ArrayList<Social>(list.size());
...
//					}
//					if (null != ch.getUserId1() && userIdList.contains(ch.getUserId1()) == false) {
//						userIdList.add(ch.getUserId1());
//					}
//					if (null != ch.getUserId2() && userIdList.contains(ch.getUserId2()) == false) {
//						userIdList.add(ch.getUserId2());
//					}
//				}
//			}
//		}
...
//					continue;
//				}
//				icm.setMsg(EmojiUtil.emojiRecover(icm.getMsg())); // 处理emoji表情
//				List<Integer> userIds = new ArrayList<Integer>();
//				userIds.add(irm.getUserId());
//				userIds.add(irm.getUserId2());
//				// 封装对方图像
...
//					mv.setUserType(StringUtils.isBlank(cmi.getType()) ? null : Integer.valueOf(cmi.getType()) + 1);
//				}
//
//				// 基本信息结束
//				mv.setNewCount(chatMessageMap.get(irm.getChatMessageId()));// 未读消息数
//				mv.setId(Long.valueOf(irm.getUserId2()));
//				mi.setSenderID(Long.valueOf(icm.getSenderId()));
//				cmi = connectionsMiniMap.get(icm.getSenderId());
//				if (null != cmi) {
//					mi.setSenderName(cmi.getName());
//				}
//				String sendMsg = icm.getMsg();
//				mi.setContent(null != cmi ? (cmi.getName() + ":" + sendMsg) : sendMsg);
//				
//				mv.setTime(icm.getTime());
//				cmi = connectionsMiniMap.get(irm.getUserId2());// 对方id
//				if (null != cmi) {
//					mv.setTitle(cmi.getName());
//				}
//
//				mv.setOrderTime(icm.getTime());
...
//				// 查找所有成员
//				// List<Integer> userIds = new ArrayList<Integer>();
//				// List<ImMucmember> queryMUCMemberList =
//				// imMucmemberDao.getByMucid(irm.getMucId());
//				// if(queryMUCMemberList != null && queryMUCMemberList.size() !=
//				// 0) {
//				// for(int k = 0; k < queryMUCMemberList.size(); k++) {
//				// ImMucmember imuc = queryMUCMemberList.get(k);
//				// if(imuc == null) continue;
//				// userIds.add(imuc.getUserId());
//				// }
//				// }
//				// List<ConnectionsMini> listConnectionsMini =
//				// GinTongInterface.getListConnectionsMini(userIds);
//				// if(listConnectionsMini != null && listConnectionsMini.size()
//				// > 0) {
//				// List<String> listImageUrl = new
//				// ArrayList<String>(listConnectionsMini.size());
//				// for(int k = 0; k < listConnectionsMini.size(); k++) {
//				// ConnectionsMini cmi = listConnectionsMini.get(k);
//				// if (cmi == null)
//				// continue;
//				// if(cmi.getImage() != null&&listImageUrl.size()<4) {
//				// listImageUrl.add(cmi.getImage());
//				// }
//				// }
//				//
...
//						// 0-text,1-audio,2-image,3-video,4-file,5-JTContact(人脉),6-knowledge(知识),7-requirement(需求),8-机构客户,
//						// 9-机构用户(线上金桐网用户),10-个人用户(线上个人用户),11-knowledge2新知识,12-会议(conference)
//						//if (immess.getMsgType() == 5 || immess.getMsgType() == 9 || immess.getMsgType() == 10) {
//						//	message = null != cmi ? (cmi.getName() + ":" + message) : message;
//						//} else if (immess.getMsgType() == 16) {
//						//	message = immess.getMsg();
//						//} else {
//							message = null != cmi ? (cmi.getName() + ":" + message) : message;
//						//}
